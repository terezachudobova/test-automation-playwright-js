...
test.describe("Objednávka pro MŠ/ZŠ", async () => {
  test.describe("Navigace", async () => {
    test("Aplikace umožňuje uživateli v menu Pro učitele vytvoření nové objednávky pro MŠ/ZŠ", async ({ orderPage }) => {
  ...
  test.describe("Aplikace umožňuje vytvoření nové objednávky pro MŠ/ZŠ", async () => {
    ...
    test("Po vyplnění IČO do formuláře Objednávka pro MŠ/ZŠ se automaticky načte jméno odběratele a adresa odběratele z ARESu", async ({ orderPage }) => {
    ...
    test("Uživatel nemůže uložit špatně vyplněnou přihlášku", async ({ orderPage }) => {
      await orderPage.setICO(ICO);
      // TODO until ARES ('Data z ARESu se nepodařilo načíst, vyplňte je prosím ručně')
      await orderPage.fillNameAndAddress(clientName, address);
    ...
    test("Uživatel může uložit vyplněnou přihlášku na příměstský tábor", async ({ orderPage }) => {
      await orderPage.setICO(ICO);
      // TODO until ARES ('Data z ARESu se nepodařilo načíst, vyplňte je prosím ručně')
      await orderPage.fillNameAndAddress(clientName, address);

      await orderPage.setOrder(substituteName, contactName, contactPhone, contactEmail, startDate, endDate);
      await orderPage.setUrbanCamp('Odpolední', '20', '10', '2');
      await orderPage.submit();
